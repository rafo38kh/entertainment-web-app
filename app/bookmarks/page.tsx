...
export default function Bookmarks() {
  ...
  const { data: tvshowData } = useQuery<TVShowData[]>({
    queryKey: ["show"],
    queryFn: async () => {
        ...
        <ul className="grid grid-cols-2 gap-4 mb-8">
          {moveiesData &&
            ...
            moveiesData?.map((bookmark) => (
              <li key={bookmark?.id + 9} className="flex flex-col items-start ">
        ...
        <ul className="grid grid-cols-2 gap-4">
          {tvshowData &&
            ...
            tvshowData?.map((bookmark) => (
              <li key={bookmark?.id + 5} className="flex flex-col items-start ">
                <div className="relative w-full">
                  <Image
                    className="w-full rounded-lg "
                    width={100}
